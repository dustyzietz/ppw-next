...
function register_messages_routes()
{
  register_rest_route('custom/v1', '/messages', array(
    ...
    array(
      ...
      'permission_callback' => 'messages_permissions_check',
    ),
    array(
      'methods' => WP_REST_Server::DELETABLE,
...
add_action('rest_api_init', 'register_messages_routes');

function messages_permissions_check()
{
  return true;
}
...
{
  $user_id = get_current_user_id();
  $design_id = $request->get_param('design_id');
  $query_args = array('user_id' => $user_id);

  if (!empty($design_id)) {
    $query_args['design_id'] = $design_id;
  }

  $messages = get_messages_from_database($query_args);

  if (empty($messages)) {
    return new WP_Error('no_messages_found', 'No messages found matching the criteria.', array('status' => 404));
  }
...
function add_message($request)
{
  ...
  if (empty($data['message']) || empty($data['design_id'])) {
    return new WP_Error('missing_parameters', 'Required parameters are missing.', array('status' => 400));
  }
  ...
  $result = add_message_to_database($message); // Implement this function to add message to your database

  if (is_wp_error($result)) {
    return $result;
  }

  return rest_ensure_response('Message added successfully.');
}
...
function delete_message($request)
{
  ...
  if (empty($message_id)) {
    return new WP_Error('missing_parameter', 'Message ID is missing.', array('status' => 400));
  }

  $message = get_message_from_database($message_id); // Implement this function to fetch message from your database

  if (empty($message)) {
    return new WP_Error('message_not_found', 'Message not found.', array('status' => 404));
  }

  $result = delete_message_from_database($message_id); // Implement this function to delete message from your database
